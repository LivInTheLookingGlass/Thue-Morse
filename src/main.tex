...
\subsubsection{Definition 3b - Recursive Rotation}

Another way to phrase this definition is as recursive rotation. If we decompose $s$, we can instead represent it as

\begin{equation}
    \begin{aligned}
r(\mathbf{x}) &= \tuple{x_{i + 1 \mod{|\mathbf{x}|}}, \ldots} \quad \text{for } \mathbf{x} = \tuple{x_0, x_1, \ldots, x_{n-1}} \\
         t(0) &= \tuple{0} \\
         t(1) &= \tuple{0, 1} \\
         t(n) &= \bigparallel_{i=0}^1 r(t(n-1), i) \\
  T_{2,3b}(n) &= t(\ceil{\log_2(n + 1)})_n
    \end{aligned}
\end{equation}
...
\subsection{Definition 8 - Evil Numbers Derivation 1}
...
\subsection{Definition 9 - Evil Numbers Derivation 2}
...
T_{2,9}(n) &= 1 - ce(n + 1) + ce(n)
    \end{aligned}
\end{equation}

\subsection{Definition 10 - Gould's Sequence Derivation}
...
\begin{equation}
    \begin{aligned}
T_{2,10}(n) &= \text{Gould}(n) - 1 \mod{3} \\
            &= \left(\sum_{k=0}^n \left(\binom{n}{k} \mod{2} \right)\right) - 1 \mod{3}
    \end{aligned}
\end{equation}

\subsection{Definition 11 - Derivation from Blue Code}
...
\text{A193231}(n) &= \bigoplus_{i=0}^{\ceil{\log_2(n)}} \hspace{-5pt} \text{A001317}\left(i \cdot \floor{\dfrac{n \mod{2}}{2^i}}\right) \\
      T_{2,11}(n) &= \text{A193231}(n) \pmod{2}
    \end{aligned}
\end{equation}
...
\note{Make sure to search A010059 (complement of Thue-Morse) \& A001285 (Thue-Morse + 1) for additional definitions and derivations}
...
\note{Need to reference eq \ref{eq:p2d04s} in definition}
...
\note{This is tested up to $2^{20} \cdot n, n \in [2, 32]$}
