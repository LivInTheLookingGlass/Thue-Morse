...
\subsection{Definition 11 - Gould's Sequence Derivation}
...
\begin{equation}
    \label{eq:p2d11}
    \begin{aligned}
T_{2,11}(n) &= \text{Gould}(n) - 1 \mod{3} \\
            &= \left(\sum_{k=0}^n \left(\binom{n}{k} \mod{2} \right)\right) - 1 \mod{3}
    \end{aligned}
\end{equation}

\subsection{Definition 12 - Derivation from Blue Code}
...
\text{A193231}(n) &= \bigoplus_{i=0}^{\ceil{\log_2(n)}} \hspace{-5pt} \text{A001317}\left(i \cdot \floor{\dfrac{n \mod{2}}{2^i}}\right) \\
      T_{2,12}(n) &= \text{A193231}(n) \pmod{2}
    \end{aligned}
\end{equation}
